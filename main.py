...
def get_all_files_from(path: str, type: str = '.md'):
    """
            ...
            if (file.endswith(type)):
                files.append(
                    ({
                        "path": root,
                        "filename": file,
                        "links": _extract_links_from(file, root)
                    })
                )
    return files


def _extract_links_from(file: str, path: str, delim: str = "/") -> list[dict]:
...
def create_graph_from_directory(dir: str):
    G = nx.DiGraph()
    all_files_with_links = get_all_files_from(dir)
